...
user_agent = "Mozilla/5.0 (X11; CrOS x86_64 11151.29.0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/71.0.3578.49 Safari/537.36"
...
feed_list = open("/mnt/array2/Audio/podcast backup/feed_urls.txt","r")
base_directory = "/mnt/array2/Audio/podcast backup/"
...
for url in feed_list:
	...
	for episode in podcast['episodes']:
		...
		if os.path.exists(ep_path) == False:
			try:
				print (ep_path)
				...
				p = subprocess.Popen('''wget --user-agent "''' + user_agent + '''" -O "''' + temp_path + '''" "''' + ep_url + '''"''',shell=True) 
				p.communicate()
				if p.returncode == 0:
